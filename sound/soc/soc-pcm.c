				...
				struct snd_pcm_hw_params *params)
{
	...
	for (i = 0; i < rtd->num_codecs; i++)
		symmetry |= codecs[i].codec_dai->driver->symmetric_rates;
	...
	for (i = 0; i < rtd->num_codecs; i++)
		symmetry |= codecs[i].codec_dai->driver->symmetric_channels;
	...
	for (i = 0; i < rtd->num_codecs; i++)
		symmetry |= codecs[i].codec_dai->driver->symmetric_samplebits;
...
static bool soc_pcm_has_symmetry(struct snd_pcm_substream *substream)
{
	...
	for (i = 0; i < rtd->num_codecs; i++)
		symmetry = symmetry ||
			codecs[i].codec_dai->driver->symmetric_rates ||
			codecs[i].codec_dai->driver->symmetric_channels ||
			codecs[i].codec_dai->driver->symmetric_samplebits;
...
static int soc_pcm_init_runtime_hw(struct snd_pcm_substream *substream)
{
	...
	for (i = 0; i < rtd->num_codecs; i++) {
		...
		rate_max = min(rate_max, codec_stream->rate_max);
		formats &= codec_stream->formats;
		rates &= codec_stream->rates;

		if (!i)
			sig_bits = codec_stream->sig_bits;
		else if (sig_bits != codec_stream->sig_bits)
			return -EINVAL;
	}
	...
	runtime->hw.rate_min = 0;
	runtime->hw.rate_max = UINT_MAX;

	snd_pcm_limit_hw_rates(runtime);

	runtime->hw.rate_min = max(runtime->hw.rate_min, cpu_stream->rate_min);
	runtime->hw.rate_min = max(runtime->hw.rate_min, rate_min);
	runtime->hw.rate_max = min_not_zero(runtime->hw.rate_max,
			cpu_stream->rate_max);
	runtime->hw.rate_max = min_not_zero(runtime->hw.rate_max, rate_max);
	runtime->hw.channels_min = max(chan_min, cpu_stream->channels_min);
	runtime->hw.channels_max = min(chan_max, cpu_stream->channels_max);

	if (runtime->hw.formats)
		runtime->hw.formats &= formats & cpu_stream->formats;
	else
		runtime->hw.formats = formats & cpu_stream->formats;

	runtime->hw.formats = formats & cpu_stream->formats;
	runtime->hw.rates = rates & cpu_stream->rates;

	runtime->hw.rates = snd_pcm_rate_mask_intersect(rates,
		cpu_stream->rates);

	if (rates & (SNDRV_PCM_RATE_KNOT | SNDRV_PCM_RATE_CONTINUOUS))
		runtime->hw.rates |= cpu_stream->rates;
	if (cpu_stream->rates
	    & (SNDRV_PCM_RATE_KNOT | SNDRV_PCM_RATE_CONTINUOUS))
		runtime->hw.rates |= rates;
...
static int soc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	ret = soc_pcm_init_runtime_hw(substream);
	if (ret)
		goto dynamic;
...
static int soc_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	for (i = 0; i < rtd->num_codecs; i++) {
		if ((codecs[i].codec_dai->active) == 1) {
			codecs[i].codec_dai->rate = 0;
			codecs[i].codec_dai->channels = 0;
			codecs[i].codec_dai->sample_bits = 0;
		}
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		for (i = 0; i < card->num_links; i++) {
			...
			for (j = 0; j < be->num_codecs; j++) {
				codec_dai = be->codecs[j].codec_dai;
				if (codec_dai->playback_widget == widget)
					return be;
			}
		}
	} else {
		...
		for (i = 0; i < card->num_links; i++) {
			...
			for (j = 0; j < be->num_codecs; j++) {
				codec_dai = be->codecs[j].codec_dai;
				if (codec_dai->capture_widget == widget)
					return be;
			}
		}
	}
	...
	struct snd_soc_dapm_widget_list **list_)
{
	...
	list_for_each_entry(dpcm, &fe->dpcm[stream].be_clients, list_be) {
		...
		for (i = 0; i < dpcm->be->num_codecs; i++) {
			codec_dai = dpcm->be->codecs[i].codec_dai;
			widget = dai_get_widget(codec_dai, stream);
