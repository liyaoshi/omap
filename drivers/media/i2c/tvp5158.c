...
tvp5158_get_video_std(struct i2c_client *client, unsigned char core)
{
	...
	if ((i2c_read & TVP_VIDEO_STANDARD_MASK) == 0x1) {

		priv->current_std = STD_NTSC_MJ;
		v4l2_info(&priv->subdev, "NTSC video detected\n");
	} else if ((i2c_read & TVP_VIDEO_STANDARD_MASK) == 0x2) {

		priv->current_std = STD_PAL_BDGHIN;
		v4l2_info(&priv->subdev, "PAL video detected\n");
	} else {
		v4l2_err(&priv->subdev, "No video detected\n");
		priv->current_std = STD_INVALID;
	}
