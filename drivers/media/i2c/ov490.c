...
{
	u8 addr_bank_high = (reg >> 24) & 0xff;
	u8 addr_bank_low  = (reg >> 16) & 0xff;
	 ...
	 */
	if (!ret)
		ov490_reg_write16(client, OV490_BANK_HIGH, addr_bank_high);
	if (!ret)
		ov490_reg_write16(client, OV490_BANK_LOW, addr_bank_low);
	if (!ret)
		ov490_reg_write16(client, reg_addr, val);
	return ret;
}
...
{
	/* For now, hard coded resolutions for OV490 sensor */
